...
func main() {
	...
	if len(os.Args) == 1 {
		...
		// An element with the id findMe has been created in testPage.
		id := "findMe"
		fmt.Println("no input, running with default data")
		ElementByID(doc, id)
	}
	return
}
...
func ElementByID(doc *html.Node, id string) *html.Node {
	...
	switch {
	case n != nil:
		fmt.Printf("An element with the id %q was found\n", id)
	case n == nil:
		fmt.Printf("No element with the id %q could be found\n", id)
	}
	return n
}
