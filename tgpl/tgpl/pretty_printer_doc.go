...
func PrettyPrint(w io.Writer, url string) error {
	return PrettyPrintDoc(w, url, 0)
}

func PrettyPrintDoc(w io.Writer, url string, docDepth int) error {
	fname := "PrettyPrint"
	...
	forEachNodeWrite(w, doc, startElement, endElement, docDepth)
	return nil
}

// forEachNode iterates over an HTML document tree.
func forEachNodeWrite(w io.Writer, n *html.Node,
	pre, post func(w io.Writer, n *html.Node, docDepth int), docDepth int) {
	...
	for c := n.FirstChild; c != nil; c = c.NextSibling {
		forEachNodeWrite(w, c, pre, post, docDepth)
	}
...
func startElement(w io.Writer, n *html.Node, docDepth int) {
...
func endElement(w io.Writer, n *html.Node, docDepth int) {
...
func setPathRoot(a html.Attribute, docDepth int) html.Attribute {
	var path string
	for i := 0; i < docDepth; i++ {
		path += "/.."
	}
	switch a.Key {
	case "href", "src", "data":
...
func setPath(n *html.Node, docDepth int) {
	switch n.Data {
	case "a", "link", "img", "object":
		for i, a := range n.Attr {
			n.Attr[i] = setPathRoot(a, docDepth)
		}
	}
}

func startElementNode(w io.Writer, n *html.Node, docDepth int) {
	buf := bytes.Buffer{}
